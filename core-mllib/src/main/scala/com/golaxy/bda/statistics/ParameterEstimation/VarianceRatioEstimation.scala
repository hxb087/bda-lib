...
object VarianceRatioEstimation {
  ...
  def main(args: Array[String]) {
    ...
    def run(p: Params): Unit = {
      ...
      val variance1 = Statistics.colStats(dataRdd1).variance
      val variance2 = Statistics.colStats(dataRdd2).variance
      val diffBuffer = ArrayBuffer[Double]()
      for(i <- Range(0,variance1.size)){
        var difference = variance1(i) / variance2(i)
