...
import org.apache.spark.ml.util.BDAInstrumentation.instrumented
...
class IForestModel(
  ...
  private[tree] def setSummary(summary: Option[IForestSummary]): this.type = {
...
class IForest(
   ...
   */
  private[tree] def splitData(dataset: Dataset[_]): RDD[(Int, Array[Vector])] = {
   ...
   */
  private[tree] def sampleFeatures(
                                       data: Array[Vector],
                                       maxFeatures: Double,
...
//      val featureIdx = random.shuffle(0 to numFeatures - 1).take(subFeatures)
   ...
   * @return tree's root node
   */
  private[tree] def iTree(data: Array[Array[Double]],
